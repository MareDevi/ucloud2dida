...
def post_api(tid, auth, ticket1):
    ...
    data = {"ticket": ticket1, "grant_type": "third"}
    ...
    return [blade, user_id]


def extract_values(html_content):
    # 定义正则表达式模式
    # 使用正则表达式找到匹配项
    execution_value = re.search(
        r'<input\s+name="execution"\s+value="([^"]+)"', html_content
    )
...
def get_login_cookies(username, password, exe):
    ...
    try:
        ...
        response = requests.post(
            redirected_url, allow_redirects=False, cookies=response.cookies
        )
        redirected_url = response.headers["Location"]
        ...
        js_links = re.findall(
            r'<script src="([^"]+\.js)"></script>', response.content.decode("utf-8")
        )
        js_content = None
        # 下载并打印index.js的内容
        ...
        (blade, user_id) = post_api(tenant_id, auth_token, ticket)
        results = {
            "tenant_id": tenant_id,
            "user_id": user_id,
        ...
        file.close()
        #  results = [tenant_id,user_id,auth_token,blade]
        # print(results)
...
def get_co_and_sa(account, password):
    ...
    get_login_cookies(account, password, exe)
